...
import com.spring.tdfinaloas_collectivites_agricoles_finale.entity.Collectivity;
import com.spring.tdfinaloas_collectivites_agricoles_finale.entity.Member;
import com.spring.tdfinaloas_collectivites_agricoles_finale.mapper.MemberMapper;
...
public class MemberRepository {
    ...
    public List<Member> findNewMembersByCollectivityAndPeriod(String collectivityId, LocalDate from, LocalDate to) {
        List<Member> memberList = new ArrayList<>();
        try (PreparedStatement ps = connection.prepareStatement("""
            select distinct member.id, first_name, last_name, birth_date, gender,
                            phone_number, email, address, profession, occupation,
                            registration_fee_paid, membership_dues_paid
            from "member"
                join collectivity_member on member.id = collectivity_member.member_id
                join member_payment on member.id = member_payment.member_debited_id
            where collectivity_member.collectivity_id = ?
              and member_payment.creation_date >= ?
              and member_payment.creation_date <= ?
              and member.registration_fee_paid = true
            """)) {
